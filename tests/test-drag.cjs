...
const global = {stage: {
    ...
    connect: (_name, callback) => { captured = callback; return 42; },
    disconnect: id => { disconnected = id; },
}};
const savePosition = (x, y) => { saved = [x, y]; };
const start = source.indexOf('    _finishDrag(');
const end = source.indexOf('    _syncDesktopVisibility()', start);
...
const handlers = {};
const handle = {connect: (name, callback) => { handlers[name] = callback; },
